...
export async function getCustomerApplications(uuids: string[], mongoUri: string, dbName: string, outputPath: string) {
...
export async function getPostApiLogRequests(uuids: string[], mongoUri: string, dbName: string, outputPath: string) {
    const client = new MongoClient(mongoUri, {});
    await client.connect();
    const db = client.db(dbName);
